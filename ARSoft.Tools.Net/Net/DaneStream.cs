...
namespace ARSoft.Tools.Net.Net
{
    ...
    public class DaneStream : AuthenticatedStream
    {
        ...
        ///     the server
        /// </param>
        public DaneStream(Stream innerStream, IDnsSecResolver resolver, bool enforceTlsaValidation = false,
            ...
            : base(innerStream, leaveInnerStreamOpen)
        {
            ...
            _sslStream = new SslStream(innerStream, leaveInnerStreamOpen, ValidateRemoteCertificate,
        ...
        private bool ValidateCertificateByTlsa(TlsaRecord tlsaRecord, X509Certificate certificate, X509Chain chain,
            SslPolicyErrors sslPolicyErrors)
        {
            switch (tlsaRecord.CertificateUsage)
            {
                case TlsaRecord.TlsaCertificateUsage.PkixTA:
                    return chain.ChainElements.Cast<X509ChainElement>()
                               .Any(x => ValidateCertificateByTlsa(tlsaRecord, x.Certificate)) &&
                           sslPolicyErrors == SslPolicyErrors.None;

                case TlsaRecord.TlsaCertificateUsage.PkixEE:
                    return ValidateCertificateByTlsa(tlsaRecord, certificate) &&
                           sslPolicyErrors == SslPolicyErrors.None;
        ...
        private bool ValidateCertificateByTlsa(TlsaRecord tlsaRecord, X509Certificate certificate) =>
            TlsaRecord.GetCertificateAssocicationData(tlsaRecord.Selector, tlsaRecord.MatchingType, certificate)
                .SequenceEqual(tlsaRecord.CertificateAssociationData);
        ...
        /// </param>
        public void AuthenticateAsClient(string targetHost, int port, ProtocolType protocol = ProtocolType.Tcp,
            X509CertificateCollection clientCertificates = null,
            SslProtocols enabledSslProtocols = SslProtocols.Default, bool checkCertificateRevocation = false)
        {
            _tlsaRecords = _resolver.ResolveSecure<TlsaRecord>(
                DomainName.Parse("_" + port + "._" + EnumHelper<ProtocolType>.ToString(protocol).ToLower() + "." +
                                 targetHost), RecordType.Tlsa);
            _sslStream.AuthenticateAsClient(targetHost, clientCertificates ?? new X509CertificateCollection(),
        ...
        ///     checked during authentication.
        /// </param>
        public async Task AuthenticateAsClientAsync(string targetHost, int port,
            ProtocolType protocol = ProtocolType.Tcp, X509CertificateCollection clientCertificates = null,
            SslProtocols enabledSslProtocols = SslProtocols.Default, bool checkCertificateRevocation = false)
        {
            ...
            await _sslStream.AuthenticateAsClientAsync(targetHost,
                clientCertificates ?? new X509CertificateCollection(), enabledSslProtocols, checkCertificateRevocation);
        }
        ...
        public override IAsyncResult BeginRead(byte[] buffer, int offset, int count, AsyncCallback asyncCallback,
            object asyncState) =>
            _sslStream.BeginRead(buffer, offset, count, asyncCallback, asyncState);
        ...
        public override IAsyncResult BeginWrite(byte[] buffer, int offset, int count, AsyncCallback asyncCallback,
            object asyncState) =>
            _sslStream.BeginWrite(buffer, offset, count, asyncCallback, asyncState);
