...
namespace ARSoft.Tools.Net.Spf
{
    ...
    public class SenderIdRecord : SpfRecordBase
    {
        private static readonly Regex _prefixRegex =
            new Regex(@"^v=spf((?<version>1)|(?<version>2)\.(?<minor>\d)/(?<scopes>(([a-z0-9]+,)*[a-z0-9]+)))$",
        ...
        public override string ToString()
        {
            ...
            else
            {
                ...
                res.Append(string.Join(",",
                    Scopes.Where(s => s != SenderIdScope.Unknown)
                        .Select(s => EnumHelper<SenderIdScope>.ToString(s).ToLower())));
            }
        ...
        private static bool TryParsePrefix(string prefix, out int version, out int minor,
            out List<SenderIdScope> scopes)
        {
            ...
            scopes = match.Groups["scopes"].Value.Split(',')
                .Select(t => EnumHelper<SenderIdScope>.Parse(t, true, SenderIdScope.Unknown)).ToList();
