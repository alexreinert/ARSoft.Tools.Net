...
namespace ARSoft.Tools.Net.Dns.Resolver
{
    ...
    public abstract class UpdateableResolverHintStoreBase : IResolverHintStore
    {
        ...
        public void Update(IDnsResolver resolver)
        {
            ...
            zone.AddRange(resolver.Resolve<DnsKeyRecord>(DomainName.Root, RecordType.DnsKey)
        ...
        private void LoadZoneInternal(Zone zone)
        {
            ...
            RootServers = zone.Where(x => x.RecordType == RecordType.A || x.RecordType == RecordType.Aaaa)
                .Join(nameServers, x => x.Name, x => x, (x, y) => ((IAddressRecord) x).Address).ToList();
            RootKeys = zone.OfType<DnsKeyRecord>().Where(x => x.Name == DomainName.Root && x.IsSecureEntryPoint)
                .Select(x => new DsRecord(x, x.TimeToLive, DnsSecDigestType.Sha256)).ToList();
        }
