...
namespace ARSoft.Tools.Net.Dns.DnsRecord
{
    ...
    public class TlsaRecord : DnsRecordBase
    {
        ...
        public TlsaRecord(DomainName name, int timeToLive, TlsaCertificateUsage certificateUsage, TlsaSelector selector,
            TlsaMatchingType matchingType, byte[] certificateAssociationData)
        ...
        public TlsaRecord(DomainName name, int timeToLive, TlsaCertificateUsage certificateUsage, TlsaSelector selector,
            TlsaMatchingType matchingType, X509Certificate certificate)
            : base(name, RecordType.Tlsa, RecordClass.INet, timeToLive)
        {
            CertificateUsage = certificateUsage;
            Selector = selector;
        ...
        internal static byte[] GetCertificateAssocicationData(TlsaSelector selector, TlsaMatchingType matchingType,
            ...
            switch (selector)
            {
                ...
                case TlsaSelector.SubjectPublicKeyInfo:
                    selectedBytes = SubjectPublicKeyInfoFactory
                        .CreateSubjectPublicKeyInfo(DotNetUtilities.FromX509Certificate(certificate).GetPublicKey())
                        .GetDerEncoded();
                    break;
