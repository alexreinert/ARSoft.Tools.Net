...
namespace ARSoft.Tools.Net.Dns.DnsRecord
{
    ...
    public class LocRecord : DnsRecordBase
    {
        ...
        public LocRecord(DomainName name, int timeToLive, byte version, double size, double horizontalPrecision,
            double verticalPrecision, Degree latitude, Degree longitude, double altitude)
        ...
        internal override void ParseRecordData(DomainName origin, string[] stringRepresentation)
        {
            ...
            Latitude = new Degree(latNegative, groups["latd"].Value, groups["latm"].Value, groups["lats"].Value,
                groups["latms"].Value);

            var longNegative = groups["long"].Value.Equals("W", StringComparison.InvariantCultureIgnoreCase);
            Longitude = new Degree(longNegative, groups["longd"].Value, groups["longm"].Value, groups["longs"].Value,
                groups["longms"].Value);
        ...
        public class Degree
        {
            ...
            public double DecimalDegrees =>
                (IsNegative ? -1d : 1d) * (Degrees + (double) Minutes / 6000 * 100 +
        ...
        {
            var res = degrees.Degrees * 3600000 + degrees.Minutes * 60000 + degrees.Seconds * 1000 +
        ...
        private static double ConvertAltitude(int altitude) =>
            (altitude < _ALTITUDE_REFERENCE ? (_ALTITUDE_REFERENCE - altitude) * -1 : altitude - _ALTITUDE_REFERENCE) /
            100d;
