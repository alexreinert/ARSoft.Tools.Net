...
namespace ARSoft.Tools.Net.Dns.Cache
{
    ...
    internal class DnsCache
    {
        ...
        public void Add<TRecord>(DomainName name, RecordType recordType, RecordClass recordClass,
            IEnumerable<TRecord> records, DnsSecValidationResult validationResult, int timeToLive)
        ...
        public void Add(DomainName name, RecordType recordType, RecordClass recordClass,
            DnsCacheRecordList<DnsRecordBase> records, int timeToLive)
        {
            var key = new CacheKey(name, recordType, recordClass);
            _cache.TryAdd(key, new CacheValue(records, timeToLive));
        }

        public bool TryGetRecords<TRecord>(DomainName name, RecordType recordType, RecordClass recordClass,
            out List<TRecord> records)
            where TRecord : DnsRecordBase
        {
            var key = new CacheKey(name, recordType, recordClass);
        ...
        public bool TryGetRecords<TRecord>(DomainName name, RecordType recordType, RecordClass recordClass,
            out DnsCacheRecordList<TRecord> records)
            where TRecord : DnsRecordBase
        {
            var key = new CacheKey(name, recordType, recordClass);
            var utcNow = DateTime.UtcNow;
        ...
        private class CacheKey
        {
            ...
            public override bool Equals(object obj)
            {
                ...
                return _recordType == other._recordType && _recordClass == other._recordClass &&
