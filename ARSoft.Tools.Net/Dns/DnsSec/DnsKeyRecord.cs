...
namespace ARSoft.Tools.Net.Dns.DnsSec
{
    ...
    public class DnsKeyRecord : DnsRecordBase
    {
        ...
        public DnsKeyRecord(DomainName name, RecordClass recordClass, int timeToLive, DnsKeyFlags flags, byte protocol,
        ...
        internal bool Verify(byte[] buffer, int length, byte[] signature)
        {
            switch (Algorithm)
            {
                ...
                case DnsSecAlgorithm.EcDsaP256Sha256:
                    return VerifyEcDsa(new Sha256Digest(), NistNamedCurves.GetByOid(SecObjectIdentifiers.SecP256r1),
                        buffer, length, signature);

                case DnsSecAlgorithm.EcDsaP384Sha384:
                    return VerifyEcDsa(new Sha384Digest(), NistNamedCurves.GetByOid(SecObjectIdentifiers.SecP384r1),
                        buffer, length, signature);
        ...
        private bool VerifyRsa(IDigest digest, byte[] buffer, int length, byte[] signature)
        {
            ...
            int exponentLength = PublicKey[0] == 0
                ...
                : PublicKey[0];
            var moduloOffset = exponentOffset + exponentLength;
            var moduloLength = PublicKey.Length - moduloOffset;

            var parameters = new RsaKeyParameters(false, new BigInteger(1, PublicKey, moduloOffset, moduloLength),
                new BigInteger(1, PublicKey, exponentOffset, exponentLength));
        ...
        {
            var numberSize = 64 + PublicKey[0] * 8;

            var parameters = new DsaPublicKeyParameters(
        ...
        {
            var dParams = ECGost3410NamedCurves.GetByOid(CryptoProObjectIdentifiers.GostR3410x2001CryptoProA);
            var reversedPublicKey = PublicKey.Reverse().ToArray();
            var q = dParams.Curve.CreatePoint(new BigInteger(1, reversedPublicKey, 32, 32),
                new BigInteger(1, reversedPublicKey, 0, 32));
            var parameters = new ECPublicKeyParameters(q, dParams);
            ...
            return signer.VerifySignature(hash, new BigInteger(1, signature, 32, 32),
                new BigInteger(1, signature, 0, 32));
        }

        private bool VerifyEcDsa(IDigest digest, X9ECParameters curveParameter, byte[] buffer, int length,
            byte[] signature)
        {
            var digestSize = digest.GetDigestSize();

            var dParams = new ECDomainParameters(
            ...
            var q = dParams.Curve.CreatePoint(new BigInteger(1, PublicKey, 0, digestSize),
            ...
            return signer.VerifySignature(hash, new BigInteger(1, signature, 0, digestSize),
        ...
        /// <returns></returns>
        public static DnsKeyRecord CreateSigningKey(DomainName name, RecordClass recordClass, int timeToLive,
            DnsKeyFlags flags, byte protocol, DnsSecAlgorithm algorithm, int keyStrength = 0)
        {
            byte[] privateKey;
            byte[] publicKey;
            ...
            switch (algorithm)
            {
                ...
                case DnsSecAlgorithm.DsaNsec3Sha1:
                    ...
                    var dsaParamsGen = new DsaParametersGenerator();
                ...
                case DnsSecAlgorithm.EcDsaP384Sha384:
                    ...
                    var ecDsaP384EcDomainParameters = new ECDomainParameters(
                    ...
                    ecDsaPublicKey.Q.AffineYCoord.ToBigInteger().ToByteArrayUnsigned()
