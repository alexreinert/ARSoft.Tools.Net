...
namespace ARSoft.Tools.Net.Dns.DnsSec
{
    ...
    public class DnsKeyRecord : DnsRecordBase
	{
		...
		private bool VerifyRsa(IDigest digest, byte[] buffer, int length, byte[] signature)
		{
			...
			var parameters = new RsaKeyParameters(false, new BigInteger(1, PublicKey, moduloOffset, moduloLength), new BigInteger(1, PublicKey, exponentOffset, exponentLength));
		...
		private bool VerifyGost(byte[] buffer, int length, byte[] signature)
		{
			...
			var q = dParams.Curve.CreatePoint(new BigInteger(1, reversedPublicKey, 32, 32), new BigInteger(1, reversedPublicKey, 0, 32));
		...
		private bool VerifyEcDsa(IDigest digest, X9ECParameters curveParameter, byte[] buffer, int length, byte[] signature)
		{
			...
			var q = dParams.Curve.CreatePoint(new BigInteger(1, PublicKey, 0, digestSize), new BigInteger(1, PublicKey, digestSize, digestSize));
		...
		public static DnsKeyRecord CreateSigningKey(DomainName name, RecordClass recordClass, int timeToLive, DnsKeyFlags flags, byte protocol, DnsSecAlgorithm algorithm, int keyStrength = 0)
		{
			...
			switch (algorithm)
			{
				...
				case DnsSecAlgorithm.EccGost:
					var gostEcDomainParameters = ECGost3410NamedCurves.GetByOid(CryptoProObjectIdentifiers.GostR3410x2001CryptoProA);
