    ...
    public abstract class DnsMessageBase
    {
        ...
        public ReturnCode ReturnCode
        {
            ...
            set
            {
                ...
                {
                    if (ednsOptions != null) ednsOptions.ExtendedReturnCode = 0;
                }
        ...
        public bool IsEDnsEnabled
        {
            ...
            set
            {
                ...
                {
                    if (_additionalRecords == null) _additionalRecords = new List<DnsRecordBase>();
        ...
        internal static DnsMessageBase CreateByFlag(byte[] data, DnsServer.SelectTsigKey tsigKeySelector,
            byte[] originalMac)
        {
            var flagPosition = 2;
            var flags = ParseUShort(data, ref flagPosition);
        ...
        internal static TMessage Parse<TMessage>(byte[] data)
            ...
            Parse<TMessage>(data, null, null);

        internal static TMessage Parse<TMessage>(byte[] data, DnsServer.SelectTsigKey tsigKeySelector,
            byte[] originalMac)
            where TMessage : DnsMessageBase, new()
        {
        ...
        {
            byte[] keyData;
            if (TSigOptions.Algorithm == TSigAlgorithm.Unknown || tsigKeySelector == null ||
                (keyData = tsigKeySelector(TSigOptions.Algorithm, TSigOptions.Name)) == null)
                return ReturnCode.BadKey;
            if (TSigOptions.TimeSigned - TSigOptions.Fudge > DateTime.Now ||
                TSigOptions.TimeSigned + TSigOptions.Fudge < DateTime.Now)
                return ReturnCode.BadTime;
            if (TSigOptions.Mac == null || TSigOptions.Mac.Length == 0)
            ...
            var maxLength = TSigOptions.StartPosition + TSigOptions.MaximumLength;
            if (originalMac != null) maxLength += originalMac.Length;

            var validationBuffer = new byte[maxLength];
            ...
            EncodeDomainName(validationBuffer, 0, ref currentPosition,
                TSigAlgorithmHelper.GetDomainName(TSigOptions.Algorithm), null, false);
            ...
            return hashAlgorithm.ComputeHash(validationBuffer, 0, currentPosition).SequenceEqual(TSigOptions.Mac)
                ? ReturnCode.NoError
                : ReturnCode.BadSig;
        }
        ...
        private static void ParseSection(byte[] resultData, ref int currentPosition, List<DnsRecordBase> sectionList,
            int recordCount)
        {
            for (var i = 0; i < recordCount; i++) sectionList.Add(ParseRecord(resultData, ref currentPosition));
        }
        ...
        private static DnsRecordBase ParseRecord(byte[] resultData, ref int currentPosition)
        {
            ...
            var name = ParseDomainName(resultData, ref currentPosition);
            var recordType = (RecordType) ParseUShort(resultData, ref currentPosition);
            var record = DnsRecordBase.Create(recordType, resultData, currentPosition + 6);
        ...
        {
            var res = Encoding.ASCII.GetString(resultData, currentPosition, length);
        ...
        {
            var res = ParseDomainName(resultData, currentPosition, out var firstLabelLength);
        ...
        internal static int ParseInt(byte[] resultData, ref int currentPosition)
        {
            ...
            if (BitConverter.IsLittleEndian)
                res = resultData[currentPosition++] << 24 | resultData[currentPosition++] << 16 |
                      resultData[currentPosition++] << 8 | resultData[currentPosition++];
            else
                res = resultData[currentPosition++] | resultData[currentPosition++] << 8 |
                      resultData[currentPosition++] << 16 | resultData[currentPosition++] << 24;
        ...
        internal static uint ParseUInt(byte[] resultData, ref int currentPosition)
        {
            ...
            if (BitConverter.IsLittleEndian)
                res = (uint) resultData[currentPosition++] << 24 | (uint) resultData[currentPosition++] << 16 |
                      (uint) resultData[currentPosition++] << 8 | resultData[currentPosition++];
            else
                res = resultData[currentPosition++] | (uint) resultData[currentPosition++] << 8 |
                      (uint) resultData[currentPosition++] << 16 | (uint) resultData[currentPosition++] << 24;
        ...
        private static DomainName ParseDomainName(byte[] resultData, int currentPosition,
            out int uncompressedLabelBytes)
        {
            var labels = new List<string>();

            var isInUncompressedSpace = true;
            uncompressedLabelBytes = 0;

            for (var i = 0; i < 127; i++) // max is 127 labels (see RFC 2065)
            {
                var currentByte = resultData[currentPosition++];
                if (currentByte == 0)
                {
                ...
                else if (currentByte == 65)
                {
                    ...
                    do
                    {
                        ...
                        if (length < 8) currentByte &= (byte) (0xff >> 8 - length);
        ...
        internal int Encode(bool addLengthPrefix, out byte[] messageData) =>
            Encode(addLengthPrefix, null, false, out messageData, out _);

        internal int Encode(bool addLengthPrefix, byte[] originalTsigMac, out byte[] messageData) =>
            Encode(addLengthPrefix, originalTsigMac, false, out messageData, out _);

        internal int Encode(bool addLengthPrefix, byte[] originalTsigMac, bool isSubSequentResponse,
            out byte[] messageData, out byte[] newTSigMac)
        {
            PrepareEncoding();

            var offset = 0;
            var messageOffset = offset;
            var maxLength = addLengthPrefix ? 2 : 0;
            ...
            var domainNames = new Dictionary<DomainName, ushort>();
            ...
            foreach (var question in Questions) question.Encode(messageData, offset, ref currentPosition, domainNames);
            foreach (var record in AnswerRecords) record.Encode(messageData, offset, ref currentPosition, domainNames);
            foreach (var record in AuthorityRecords)
                record.Encode(messageData, offset, ref currentPosition, domainNames);
            foreach (var record in _additionalRecords)
                record.Encode(messageData, offset, ref currentPosition, domainNames);
            ...
            else
            {
                ...
                var tsigVariablesPosition = currentPosition;
                ...
                else
                {
                    ...
                    EncodeDomainName(messageData, offset, ref tsigVariablesPosition,
                        TSigAlgorithmHelper.GetDomainName(TSigOptions.Algorithm), null, false);
                    TSigRecord.EncodeDateTime(messageData, ref tsigVariablesPosition, TSigOptions.TimeSigned);
                    EncodeUShort(messageData, ref tsigVariablesPosition, (ushort) TSigOptions.Fudge.TotalSeconds);
            ...
            Dictionary<DomainName, ushort> domainNames, bool useCanonical)
        {
            while (true)
            {
                ...
                var label = useCanonical ? name.Labels[0].ToLowerInvariant() : name.Labels[0];

                if (isCompressionAllowed) domainNames[name] = (ushort) ((currentPosition | 0xc000) - offset);
        ...
        {
            if (data != null) EncodeByteArray(messageData, ref currentPosition, data, data.Length);
        }
