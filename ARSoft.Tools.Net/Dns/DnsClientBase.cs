...
namespace ARSoft.Tools.Net.Dns
{
    public abstract class DnsClientBase
    {
        ...
        {
            PrepareMessage(message, out var messageLength, out var messageData, out var tsigKeySelector,
                out var tsigOriginalMac);
            ...
            for (var i = 0; i < endpointInfos.Count; i++)
            {
                ...
                try
                {
                    ...
                    var resultData = sendByTcp
                        ? QueryByTcp(endpointInfo.ServerAddress, messageData, messageLength, ref tcpClient,
                            ref tcpStream, out var responderAddress)
                    ...
                    if (resultData != null)
                    {
                        ...
                        {
                            resultData = QueryByTcp(responderAddress, messageData, messageLength, ref tcpClient,
                                ref tcpStream, out responderAddress);
                            if (resultData != null)
                            {
                                TMessage tcpResult;
                        ...
                        {
                            resultData = QueryByTcp(responderAddress, null, 0, ref tcpClient, ref tcpStream,
        ...
        {
            using (var udpClient =
                new Socket(endpointInfo.LocalAddress.AddressFamily, SocketType.Dgram, ProtocolType.Udp))
            {
                try
                {
        ...
        private byte[] QueryByTcp(IPAddress nameServer, byte[] messageData, int messageLength, ref TcpClient tcpClient,
            ref NetworkStream tcpStream, out IPAddress responderAddress)
        {
            ...
            where TMessage : DnsMessageBase, new()
        {
            ...
            for (var i = 0; i < endpointInfos.Count; i++)
            {
                ...
                try
                {
                    ...
                    if (result.IsTcpResendingRequested)
                    {
                        resultData = await QueryByTcpAsync(resultData.ResponderAddress, messageData, messageLength,
                            resultData.TcpClient, resultData.TcpStream, token);
                        if (resultData != null)
                        {
                            TMessage tcpResult;
                    ...
                    while (isTcpNextMessageWaiting)
                    {
                        ...
                        resultData = await QueryByTcpAsync(resultData.ResponderAddress, null, 0, resultData.TcpClient,
                            resultData.TcpStream, token);
                        if (resultData != null)
                        {
                            TMessage tcpResult;
        ...
        private async Task<QueryResponse> QueryByTcpAsync(IPAddress nameServer, byte[] messageData, int messageLength,
            TcpClient tcpClient, NetworkStream tcpStream, CancellationToken token)
        {
            if (!IsTcpEnabled)
                return null;
